...
// Tests for func folksonomyTags(for productType: OFFProductType, with barcode: OFFBarcode, by owner: String?) -> URL

class FSNMUrlTagsTests: XCTestCase {
    ...
    func testProduct() throws {
        ...
        let url = URL.folksonomyTags(for: .food, with: OFFBarcode(barcode: barcodeToTest), by: nil)
    ...
    func testProductBeauty() throws {
        ...
        let url = URL.folksonomyTags(for: .beauty, with: OFFBarcode(barcode: barcodeToTest), by: nil)
        XCTAssertEqual(url.description, result)
    }

    func testProductPetFood() throws {
        let result = "https://api.folksonomy.openpetfoodfacts.org/product/" + barcodeToTest
        let url = URL.folksonomyTags(for: .petFood, with: OFFBarcode(barcode: barcodeToTest), by: nil)
        XCTAssertEqual(url.description, result)
    }

    func testProductProduct() throws {
        let result = "https://api.folksonomy.openproductfacts.org/product/" + barcodeToTest
        let url = URL.folksonomyTags(for: .product, with: OFFBarcode(barcode: barcodeToTest), by: nil)
        XCTAssertEqual(url.description, result)
    }

    func testProductOwner() throws {
        let result = "https://api.folksonomy.openfoodfacts.org/product/" + barcodeToTest + "?owner=" + owner
        let url = URL.folksonomyTags(for: .food, with: OFFBarcode(barcode: barcodeToTest), by: owner)
        XCTAssertEqual(url.description, result)
    }
